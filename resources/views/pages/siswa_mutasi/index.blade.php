...
@section('subtitle', 'Siswa Mutasi Kelas')
@section('content_header_title', 'Home')
@section('content_header_subtitle', 'Siswa Mutasi Kelas')
...
<x-adminlte-card title="Form Siswa Mutasi Kelas">
    <form action="" method="post" id="formData">
        ...
        <div class="row">
            <x-adminlte-input name="asal_sekolah" label="Asal Sekolah" placeholder="Tulis Asal Sekolah" id="asal_sekolah"
                fgroup-class="col-md-6" disable-feedback enable-old-support error-key value="{{ old('asal_sekolah') }}"/>

            <x-adminlte-select name="kelas_tujuan" label="Kelas Tujuan" fgroup-class="col-md-6">
                <x-adminlte-options :options="$kelas"
                selected="{{ old('kelas_tujuan') }}" empty-option=".:: Pilih Kelas Tujuan ::."/>
            </x-adminlte-select>
        </div>
    ...
    <script>
        $('#formData').submit(function (e) {
            ...
            if (!$('#kelas_tujuan').val()) {
                Swal.fire({
                    title: 'Perhatian!',
                    text: 'Kelas tujuan harus dipilih',
                    icon: 'warning'
                });
                return false;
            }
            ...
            $.post("{{ route('siswa_mutasi.store') }}", data)
            .done(function (response) {
                if(response.success){
                    ...
                    setTimeout(() => {
                        location.href = "{{ route('siswa_mutasi.main') }}"
                    }, timer);
                }else{
