...
<body>
    ...
    <div class="header">
        ...
        <h3>periode (range bulan, jika semua bulan tidak perlu) tahun ajaran {{ $tahun_ajaran }}</h3>
    </div>
    ...
    <table>
        ...
        <tbody>
            <!-- Pos Pemasukan -->
            @php $pemasukanItems = $data['items']; @endphp
            @if($pemasukanItems->count() > 0)
                @foreach($pemasukanItems as $item)
                <tr class="pemasukan-section">
                    <td class="item-name">(item pos pemasukan) {{ $item['pos_item']->nama_pos_pemasukan }}</td>
            ...
            <!-- Pos Pengeluaran -->
            @php $pengeluaranItems = $data['items']; @endphp
            @if($pengeluaranItems->count() > 0)
                @foreach($pengeluaranItems as $item)
                <tr class="pengeluaran-section">
                    <td class="item-name">(item pos pengeluaran) {{ $item['pos_item']->pos_pengeluaran }}</td>
                    @foreach($item['periods'] as $period)
